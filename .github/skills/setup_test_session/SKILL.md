...
4. Abrir el browser y navegar a la URL seleccionada usando `mcp_playwright_browser_navigate`.
5. Limpiar `localStorage` y `sessionStorage` usando `mcp_playwright_browser_evaluate`.
6. Recargar la página para forzar la pantalla de login.
7. Completar login usando herramientas MCP (click, type) con los selectores de `.vscode/config.md`.
